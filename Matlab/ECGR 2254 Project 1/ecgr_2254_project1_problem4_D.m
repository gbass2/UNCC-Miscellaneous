...
beta = 0.5 * 10^(-5);
delta_t = 6/2000000;
t = 0:delta_t:10; % Replace 10 with 50 in part e
...
Theta = [0];
omega = [0];
Kadj = 0.1;              % Equal 1 in part e and .1 in part f
ThetaRef = pi/4;
...
subplot(3,1,1);
plot(t,ia);
title('Problem 4.D Current vs Time');
xlabel('Time(s)');
ylabel('Current(A)');
subplot(3,1,2);
plot(t,omega);
title('Problem 4.D Speed vs Time');
xlabel('Time(s)');
ylabel('Speed');
subplot(3,1,3);
plot(t,Theta);
title('Problem 4.D Angle vs Time');
xlabel('Time(s)');
ylabel('Angle');