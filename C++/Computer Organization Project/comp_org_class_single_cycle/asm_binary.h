...
class RegisterFile{
  ...
  int regPos; // Position of the register Ri in the value vector. Used when writing back to the register
...
class AsmBinary : public RegisterFile, public DataMemory, public Opcode{
  public:
  // Used for outputting the asm. Milesone 2
  ...
  int numberLocation = 0; // Used in the set or power function. Used as an iterator for accessing #numbers
  
  int iterator = 0; // An iterator for the for loop that executes the instruction
