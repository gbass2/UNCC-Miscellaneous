...
string AsmBinary::CONVERT_ASM_TO_BINARY(){
    if(!R1.empty() || !R2.empty() || !R3.empty()){
        ...
        // Used for outputting the ASM for milestone 2
...
string RegisterFile::GET_VALUE(string reg){
    int regPos;
    ...
    this-> regPos = regPos; // Setting the position of Ri into a variable for writing back to the register later
    
    return value.at(regPos);
}

void RegisterFile::WRITE_VALUE(string result){
    // Writing the result into Ri
    value.at(regPos) = result;
}